...
test('account security covers password, setup, link, unlink, and revocation boundaries', async ({ browser, page }) => {
  ...
  await check('provider-link-success', linkPage.getByText('Google sign-in could not be completed. Try again or sign in with your password.', { exact: true })).toBeVisible()
  await linkPage.goto('/account#login-methods')
  ...
  await check('provider-unlink-success-revocation', linkPage).toHaveURL(/\/auth\/oauth\/callback\?oauth_error=account_link_required/)
  await check('provider-unlink-success-revocation', linkPage.getByText('This email already has a password account. Sign in with your password, then link Google from Account Security.', { exact: true })).toBeVisible()
