...
test('the OAuth callback renders a safe provider failure without exposing details', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Signing you in', exact: true })).toBeVisible()
  await expect(page.getByText('Google sign-in could not be completed.', { exact: true })).toBeVisible()
