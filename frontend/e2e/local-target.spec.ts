...
test('local target goes from connectivity check to a passing screenshot run', async ({ page }) => {
  ...
  const navigationStep = page.locator('.step-card').nth(0)
  await navigationStep.getByRole('spinbutton', { name: 'Viewport width' }).fill('1280')
  await navigationStep.getByRole('spinbutton', { name: 'Viewport height' }).fill('720')
  await navigationStep.getByRole('textbox', { name: 'Locale' }).fill('en-US')
  await navigationStep.getByRole('textbox', { name: 'Timezone' }).fill('Asia/Ho_Chi_Minh')
