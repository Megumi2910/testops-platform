...
export function SuitesPage() {
  ...
  return <section className="page-stack">
    ...
    {query.isError && <ProjectErrorAlert title="Unable to load suites." error={query.error} fallback="Try again after the backend is ready." retryLabel="Reload suites" onRetry={() => void query.refetch()} />}
    {query.data?.length
...
export function SuitePage() {
  ...
  if (suiteQuery.isError || !suite) return <ProjectErrorAlert title="Unable to load this suite." error={suiteQuery.error} fallback="The suite may not exist in this project." retryLabel="Reload suite" onRetry={() => void suiteQuery.refetch()} />
  ...
  return <Card>
    ...
    {casesQuery.isError && <ProjectErrorAlert title="Unable to load test cases." error={casesQuery.error} fallback="Try again after the backend is ready." retryLabel="Reload test cases" onRetry={() => void casesQuery.refetch()} />}
    {casesQuery.data?.length
      ...
      : casesQuery.data
        ...
        : null}
    <ConfirmDialog open={trashOpen} title={`Move ${suite.name} to Trash?`} description="The suite and its cases become read-only and cannot run. Existing execution history remains available." confirmLabel="Move to trash" busy={archive.isPending} onClose={() => { setTrashOpen(false); archive.reset() }} onConfirm={() => archive.mutate()}>
      {archive.isError && <ProjectErrorAlert title="Unable to move this suite to Trash." error={archive.error} fallback="The suite remains active." retryLabel="Retry move to trash" busy={archive.isPending} onRetry={() => archive.mutate()} />}
    </ConfirmDialog>
