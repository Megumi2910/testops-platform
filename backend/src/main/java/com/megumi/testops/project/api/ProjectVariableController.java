...
public class ProjectVariableController {
    ...
    @DeleteMapping("/{key}") @ResponseStatus(HttpStatus.NO_CONTENT) public void delete(@AuthenticationPrincipal Jwt jwt, @PathVariable UUID projectId, @PathVariable String key, @RequestParam(required = false) Long projectVersion) { service.delete(jwt, projectId, key, projectVersion); }
}
