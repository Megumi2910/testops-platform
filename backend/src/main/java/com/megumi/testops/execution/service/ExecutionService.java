...
public class ExecutionService {
    ...
    public List<ExecutionDtos.ExecutionResponse> list(Jwt jwt, UUID projectId) { UserEntity user = access.user(jwt); ProjectEntity project = access.project(projectId); if (!access.globalAdmin(jwt)) access.membership(project, user); return executions.findByProjectIdOrderByCreatedAtDesc(projectId).stream().map(this::response).toList(); }
    @Transactional(readOnly = true)
    ...
    private ExecutionDtos.ExecutionResponse response(ExecutionEntity e) { List<ExecutionDtos.CaseResultResponse> caseDtos = caseResults.findByExecutionIdOrderByTestCase_NameAsc(e.getId()).stream().map(this::caseResponse).toList(); List<ExecutionDtos.ArtifactResponse> artifactDtos = artifacts.findByExecutionIdOrderByCreatedAtAsc(e.getId()).stream().map(a -> new ExecutionDtos.ArtifactResponse(a.getId(), a.getCaseResultId(), a.getType(), a.getContentType(), a.getByteSize(), a.getSha256(), a.isSecretSuppressed(), a.getCreatedAt())).toList(); return new ExecutionDtos.ExecutionResponse(e.getId(), e.getProject().getId(), e.getSuite() == null ? null : e.getSuite().getId(), e.getStatus().name(), e.getTotalCases(), e.getCompletedCases(), e.getPassedCases(), e.getFailedCases(), e.getErrorCases(), e.getCancelledCases(), e.getCreatedAt(), e.getStartedAt(), e.getFinishedAt(), e.getErrorMessage(), caseDtos, artifactDtos); }
    private ExecutionDtos.CaseResultResponse caseResponse(TestCaseResultEntity result) { return new ExecutionDtos.CaseResultResponse(result.getId(), result.getTestCase().getId(), result.getTestCase().getName(), result.getStatus().name(), result.getAttemptCount(), result.getStartedAt(), result.getFinishedAt(), result.getErrorMessage(), stepResults.findByCaseResultIdOrderByPositionAsc(result.getId()).stream().map(s -> new ExecutionDtos.StepResultResponse(s.getPosition(), s.getAction(), s.getStatus(), s.getDurationMs(), s.getErrorMessage())).toList()); }
