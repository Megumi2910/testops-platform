...
| Cart/checkout/orders/reviews | Selective checkout, server-side pricing, idempotency replay, cart cleanup, and one-time cancellation now pass in native and isolated browser contracts | PARTIAL | `QG-B13` PostgreSQL final-unit concurrency, payment-state, stale-stock, and review scenarios remain |
