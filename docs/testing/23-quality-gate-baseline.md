...
| Members | Backend/HTTP/PostgreSQL protect scope/invariants; rebuilt PM UI exposes labelled versioned changes and confirmed removal; desktop/mobile PM plus direct-URL viewer DevTools journeys are clean and viewer is read-only | PARTIAL | `QG-B07` positive add/remove/duplicate API flows plus test-manager/tester/non-member/admin HTTP/browser journeys remain |
