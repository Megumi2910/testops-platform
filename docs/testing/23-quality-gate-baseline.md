...
| --- | --- | --- | --- |
| TestOps | `16411c4ab9c5ec3fbea815580f40af26fc49cd20` | Isolated `testops-quality-gate` backend/frontend OCI `org.opencontainers.image.revision` labels match; PostgreSQL, Mailpit, pgAdmin, backend, and frontend report healthy; health/UI probes return 200 | PASS |
...
| Responsive home | Chrome mobile emulation at `320×800`; no transport failure; semantic tree captured; category cards, product/category links, shared-header controls, and the product-gallery zoom dialog now expose native semantics | PARTIAL | `QG-006` remaining route/form semantics and full mobile keyboard matrix still need closure; see `docs/testing/71-phase5-ecommerce-gallery-accessibility-evidence.md` |
...
| Accessibility | Previous mobile Lighthouse accessibility `80`; best practices `81`; SEO `92`; local header/catalog contract plus product-gallery dialog contract now pass | FAIL | `QG-006` remaining route/form findings and `QG-007` Lighthouse >=95 must be rerun after the full remediation |
...
| Cart/checkout/orders/reviews | Selective checkout, server-side pricing, idempotency replay, cart cleanup, one-time cancellation, two-user final-unit stock reservation, backend-owned QR configuration, stale-cart recovery, completed-purchase review creation, duplicate rejection, and non-purchaser rejection now pass in native and isolated browser contracts | PARTIAL | `QG-B13` payment capture/webhooks and the broader checkout accessibility matrix remain; see `docs/testing/67-phase5-payment-stale-stock-evidence.md` and `docs/testing/69-phase5-ecommerce-review-evidence.md` |
| Messaging | Separate customer-A/seller-B and customer-B/seller-B threads are seeded; two independent contexts prove live `/ws` delivery, disconnected REST fallback, unread filtering, and read-state removal after opening the thread | PARTIAL | `QG-B14` backend-restart reconnect timing, native multi-user stress, and the complete role/thread matrix remain; see `docs/testing/68-phase5-ecommerce-messaging-resilience-evidence.md` |
| Seller/admin boundaries | Two verified customers, two approved sellers, administrator, and product state fixtures are seeded idempotently; Seller A/B product-list isolation passed in two contexts; the permission matrix proves guest/unverified restrictions, customer-to-seller/admin denial, seller-to-admin denial, non-disclosing foreign-product mutation rejection, and administrator read surfaces | PARTIAL | `QG-B06` seller/admin write workflows, seller order ownership transitions, review ownership, and complete endpoint/UI matrix remain |
| Unfinished destinations | Wishlist and Flash Sale routes now expose explicit unavailable status, preserve a catalog recovery action, and disable empty-state controls; wallet/voucher cards and header integrations retain coming-soon/disabled copy | PARTIAL | `QG-009` remaining simulated settings and any unreviewed destination need the same truthful-state audit; see `docs/testing/70-phase5-ecommerce-feature-availability-evidence.md` |
