...
| Role and tenant browser matrix | `phase5-role-matrix.spec.ts` registers QA users, grants TEST_MANAGER/TESTER/VIEWER roles, verifies create/run/member controls, rejects a non-member project load, and captures a `404` for a foreign suite ID while preserving the legitimate suite | PASS | Administrator, unverified, session, dashboard, and execution-artifact variants remain in `QG-B01`, `QG-B02`, `QG-B05`, `QG-B08`, `QG-B09`, and `QG-B10` |
