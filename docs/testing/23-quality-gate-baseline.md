...
| Messaging | Separate customer-A/seller-B and customer-B/seller-B threads are seeded; customer B has an independent cart/order; cross-customer thread reads passed in two browser contexts | PARTIAL | `QG-B14` live WebSocket/reconnect/REST-fallback send/receive orchestration remains |
