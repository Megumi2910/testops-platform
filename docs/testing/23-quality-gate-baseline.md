...
| Executions | Archived suites are blocked from queueing; requester/manager/denied cancellation and foreign execution IDs have service plus HTTP problem coverage; the browser matrix now proves cooperative cancellation, infrastructure retry count, and failure-category persistence | PARTIAL | `QG-B08` worker/queue-full, secret suppression, target-escape, browser-crash, and full artifact matrix remain |
