...
| Members | Fixture roles render; response permission sets and backend guards cover PM/test-manager/tester/viewer/non-member/admin | PARTIAL | `QG-B07` HTTP add/change/remove/final-manager and cross-project substitutions remain |
...
The original baseline backend verification passed with 59 tests. After the Phase 2–5 repairs, the current backend unit/package gate passes 101 tests, including four dashboard aggregate tests, 19 project-permission matrix additions, and four fail-closed QA fixture property tests. The isolated PostgreSQL gate additionally passes all 6 `ApplicationContextIT` cases on a clean V021 schema. Both Compose configurations parsed, both stacks became healthy, and all four application images matched their checked-out revisions at baseline capture.
