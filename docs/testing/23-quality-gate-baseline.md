...
| Variables | Listing now requires advertised `VARIABLE_VIEW`; secret values remain masked for every API consumer; the evidence-safety browser journey proves secret cases suppress screenshot/trace artifacts while ordinary variables retain them | PARTIAL | `QG-B05` failure/browser-artifact variants and full download authorization remain |
...
| Executions | Archived suites are blocked from queueing; requester/manager/denied cancellation and foreign execution IDs have service plus HTTP problem coverage; the browser matrix proves cooperative cancellation, infrastructure retry count, failure-category persistence, secret/non-secret evidence behavior, and click/form target escape; `ExecutionWorkerTest` and `ExecutionServiceTest` prove disabled polling and pre-persistence queue-full rejection | PARTIAL | `QG-B08` browser-crash, secret-bearing failure variants, and complete artifact-download matrix remain; deployment-mode behavior still needs Chrome DevTools evidence |
