...
| Members | Backend/HTTP/PostgreSQL protect scope/invariants; HTTP and service positive add/change/remove, duplicate, archived-project, final-manager, and stale-version paths are covered; rebuilt PM UI exposes labelled versioned changes and confirmed removal; desktop/mobile PM plus direct-URL viewer DevTools journeys are clean and viewer is read-only | PARTIAL | `QG-B07` browser role matrix remains |
