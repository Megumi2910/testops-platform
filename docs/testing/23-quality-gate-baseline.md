...
| Cart/checkout/orders/reviews | Selective checkout, server-side pricing, idempotency replay, cart cleanup, one-time cancellation, two-user final-unit stock reservation, backend-owned QR configuration, and stale-cart recovery now pass in native and isolated browser contracts | PARTIAL | `QG-B13` payment capture/webhooks, review scenarios, and the broader checkout accessibility matrix remain; see `docs/testing/67-phase5-payment-stale-stock-evidence.md` |
| Messaging | Separate customer-A/seller-B and customer-B/seller-B threads are seeded; customer B has an independent cart/order; cross-customer thread reads passed in two browser contexts; two independent contexts now prove a Customer A message reaches Seller B over `/ws` while the REST send is intentionally aborted | PARTIAL | `QG-B14` reconnect timing, explicit REST-fallback delivery, unread-state, and native multi-user stress coverage remain |
