...
| Seller/admin boundaries | Two verified customers, two approved sellers, administrator, and product state fixtures are seeded idempotently; Seller A/B product-list isolation passed in two contexts | PARTIAL | `QG-B06` write authorization, admin mutations, and complete endpoint/UI matrix still require execution |
