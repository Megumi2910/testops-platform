...
| Members | Fixture roles render; response permissions and backend guards cover all roles; service and HTTP contracts reject final-manager removal/demotion and hide foreign member IDs | PARTIAL | `QG-B07` positive/stale/archived membership mutations plus persisted multi-role HTTP/browser journeys remain |
