...
| Projects | Two fixture projects render; target project opens; versioned project archive/restore and archived-project definition blocking are covered by `definition-lifecycle.spec.ts`; the Phase 5 role matrix also proves member/non-member project boundaries and nested suite substitution; the unverified recovery journey proves restricted project navigation after login | PARTIAL | Edit and duplicate-name variants remain; Google/session variants remain under `QG-B01` and `QG-B02` |
