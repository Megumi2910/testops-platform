...
| --- | --- | --- | --- |
| Authentication | Manager browser login; API login across every seeded role/state; protected app load and refresh; enumeration-safe OTP resend, server countdown, concurrent/reload idempotency, invalid-code, protected return-URL, and Mailpit verification are covered by `phase5-auth-session-matrix.spec.ts` | PARTIAL | `QG-B01` time-controlled expiry and password recovery remain |
...
| Role and tenant browser matrix | `phase5-role-matrix.spec.ts` covers project roles and tenant scope; `phase5-auth-session-matrix.spec.ts` covers invalid OTP, protected return navigation, and two-browser session revoke/revoke-all | PARTIAL | Administrator, locked/disabled, dashboard, execution-artifact, and complete accessibility/performance variants remain in `QG-B01`, `QG-B02`, `QG-B05`, `QG-B08`, `QG-B09`, and `QG-B10` |
| Executions | Archived suites are blocked from queueing; requester/manager/denied cancellation and foreign execution IDs have service plus HTTP problem coverage; the browser matrix proves cooperative cancellation, infrastructure retry count, and failure-category persistence; `ExecutionWorkerTest` and `ExecutionServiceTest` prove disabled polling and pre-persistence queue-full rejection | PARTIAL | `QG-B08` secret suppression, target-escape, browser-crash, and full artifact matrix remain; deployment-mode behavior still needs browser-level artifact evidence |
...
| Administration | Frontend route requires `USER_ADMINISTER`; transactional last-active-admin protection has focused coverage; browser matrix proves guest login redirect and verified-member denial | PARTIAL | `QG-B10` bootstrap-administrator CRUD and full role matrix remain |
