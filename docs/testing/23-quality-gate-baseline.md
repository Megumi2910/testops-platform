...
| Members | Backend/HTTP/PostgreSQL protect scope/invariants; positive add/change/remove, duplicate, archived-project, final-manager, and stale-version service paths are covered; rebuilt PM UI exposes labelled versioned changes and confirmed removal; desktop/mobile PM plus direct-URL viewer DevTools journeys are clean and viewer is read-only | PARTIAL | `QG-B07` browser role matrix and HTTP-visible positive add/remove/duplicate journeys remain |
