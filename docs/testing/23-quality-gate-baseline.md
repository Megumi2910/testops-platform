...
| Projects | Two fixture projects render; target project opens; versioned project archive/restore and archived-project definition blocking are covered by `definition-lifecycle.spec.ts` | PARTIAL | Edit, duplicate, and full project permission matrix remain under the broader Phase 5 role/browser matrix; project lifecycle stale/conflict behavior is covered by `testing/39-project-lifecycle-version-evidence.md` |
