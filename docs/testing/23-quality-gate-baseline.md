...
| Cart/checkout/orders/reviews | Selective checkout, server-side pricing, idempotency replay, cart cleanup, one-time cancellation, and two-user final-unit stock reservation now pass in native and isolated browser contracts | PARTIAL | `QG-B13` payment-state, stale-stock UI, and review scenarios remain |
