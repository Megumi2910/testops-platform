...
| Members | Fixture roles render; response permissions and backend guards cover all roles; service/HTTP contracts hide foreign IDs; PostgreSQL proves valid demotion, final-manager retention, stale versions, and archived-project blocking | PARTIAL | `QG-B07` positive add/remove/duplicate flows plus persisted multi-role HTTP/browser journeys remain |
