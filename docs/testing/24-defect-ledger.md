...
| QG-B07 | remaining membership HTTP/browser matrix | final-manager demotion/removal, foreign member IDs, nested case IDs, foreign execution IDs, cancellation ownership, response permissions, and role guards now have focused service/MockMvc coverage; add persisted positive/stale/archived mutations and Chrome DevTools role journeys |
