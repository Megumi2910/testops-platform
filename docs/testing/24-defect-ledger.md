...
| QG-B06 | ecommerce cross-customer/cross-seller/admin isolation | PARTIAL: dev seeder provides Customer A/B, Seller A/B, administrator, stateful products, separate carts/orders, and stable ownership data; `phase5-ecommerce-role-isolation.spec.ts` proves customer list/nested-read isolation and Seller A/B product-list isolation, while write/admin matrix remains |
