...
- Resolution: `docker-compose.e2e.yml` now sets `PROJECT_SECRET_VARIABLES_ENABLED=true` and `PROJECT_VARIABLE_KEY_PATH=/run/secrets/testops/project-variable-key`, using the ephemeral key generated by the CI preparation script
- Verification: local isolated stack already passes the two evidence-safety cases; the next CI run must confirm the corrected profile
