...
| QG-B07 | remaining membership HTTP/browser matrix | focused service/MockMvc covers ancestry, cancellation, and final manager; PostgreSQL additionally proves valid manager demotion, persisted count, final-manager retention, stale versions, and archived-project blocking; add positive add/remove/duplicate workflows and Chrome DevTools role journeys |
