...
| QG-B06 | ecommerce cross-customer/cross-seller/admin isolation | RESOLVED prerequisite: dev seeder now provides Customer A/B, Seller A/B, administrator, stateful products, separate carts/orders, and stable ownership data; endpoint/UI authorization matrix remains |
...
| QG-B14 | two-user messaging | RESOLVED prerequisite: separate customer-A/seller-B and customer-B/seller-B threads are seeded; two-browser WebSocket/reconnect/REST-fallback orchestration remains |
