...
The normal E2E Compose profile sets `PROJECT_SECRET_VARIABLES_ENABLED=true` and uses the ephemeral `project-variable-key` generated by `.github/scripts/prepare-e2e-secrets.sh`. The generator emits Base64 for 32 random bytes because the backend accepts raw 32-byte or Base64-encoded 256-bit keys. Without these explicit overrides, CI copies `backend/.env.example` and intentionally disables secret variables; with an incorrectly sized key, the backend fails health startup before the browser suite begins.
