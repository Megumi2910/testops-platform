...
Viewport width/height, locale, and IANA timezone are now carried by the first step and copied into `execution_step_snapshots`. `PlaywrightCaseRunner` translates them into `Browser.NewContextOptions` before opening the page, preserving one isolated context for the complete case. The builder keeps the controls together, and both UI and backend validation reject partial viewport pairs, unsupported dimensions, malformed locale tags, invalid timezone IDs, or settings placed on a later step. This is intentionally a first-step contract because Playwright does not support changing these context options safely after a context has been created.
