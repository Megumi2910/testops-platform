...
current manifest has 33 cases: twenty-eight safe single-browser cases are `READY`
...
cancellation-guard, seller-dashboard, seller-store-profile, and seller-analytics journeys).
...
The current catalog has 9 suites and 33 cases. Its 28-case READY set
...
and completed-order cancellation-guard journeys plus the seller dashboard, seller store profile, and seller analytics. The logout case verifies the account
...
port 8180 completed successfully for all 9 suites and 33 cases. The run
...
Target checking returned `REACHABLE` with HTTP 200; all 28 READY cases passed,
with 207 total steps. Twenty-six definitions contain a screenshot step; the
