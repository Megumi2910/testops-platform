...
current manifest has 27 cases: twenty-one safe single-browser cases are `READY`
...
invalid-login feedback, and contact-form accessibility). Credentialed verification,
...
The current catalog has 9 suites and 27 cases. Its 21-case READY set
...
mobile keyboard search, guest cart route-guard, invalid-login, and contact-form
accessibility journeys. Search uses
...
Target checking returned `REACHABLE` with HTTP 200; all 21 READY cases passed,
with 119 total steps. Nineteen screenshot-bearing cases each retained a
