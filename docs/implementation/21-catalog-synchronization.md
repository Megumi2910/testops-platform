...
port 8180 completed successfully for all 9 suites and 38 cases. The run
...
The guest admin-route guard passed all 5 steps in execution
`dfa5e7be-b46f-4ee0-ade4-674cb868d697`, confirming `/admin` redirects to
`/login` and exposes the login heading/email field; its screenshot is retained
because no credential is used.
