...
| Catalog preflight | `scripts/sync-ecommerce-catalog.ps1 -Mode dry-run` | PASS — 9 suites, 27 cases, no API calls |
| Catalog apply | `-Mode apply -BaseUrl http://localhost:8180` | PASS — 9 suites, 27 cases reconciled; variable values redacted in logs |
| Live READY acceptance | target check + four suite queue requests | PASS — target `REACHABLE`/HTTP 200, all 21 READY cases passed, 119 steps, 19 screenshot artifacts plus traces |
