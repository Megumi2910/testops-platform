...
| Catalog preflight | `scripts/sync-ecommerce-catalog.ps1 -Mode dry-run` | PASS — 9 suites, 26 cases, no API calls |
| Catalog apply | `-Mode apply -BaseUrl http://localhost:8180` | PASS — 9 suites, 26 cases reconciled; variable values redacted in logs |
| Live READY acceptance | target check + four suite queue requests | PASS — target `REACHABLE`/HTTP 200, all 20 READY cases passed, 113 steps, 18 screenshot artifacts plus traces |
...
catalog and search (10/10), authentication/customer routes (7/7), and
...
the exact `Invalid email or password` message, and retains a screenshot.
