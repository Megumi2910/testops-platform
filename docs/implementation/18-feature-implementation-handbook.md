...
Failure classification is centralized in `PlaywrightCaseRunner.category(...)`: assertion and definition errors remain test failures, while unreachable targets, browser crashes, blocked navigation, and worker timeouts become infrastructure errors. The category is copied to `TestCaseResultEntity.errorCategory` for readable recovery guidance.

The executable language also includes `PRESS`, `HOVER`, `ASSERT_VALUE`, `ASSERT_CHECKED`, `ASSERT_ENABLED`, `ASSERT_DISABLED`, `ASSERT_ATTRIBUTE`, `ASSERT_COUNT`, and `ASSERT_URL_EQUALS`. Additions update `DefinitionService`, the platform options metadata, `PlaywrightCaseRunner`, frontend builder hints/validation, and focused tests together. `ASSERT_URL_EQUALS` is target-guarded even when a case supplies a relative path; `ASSERT_ATTRIBUTE` and `ASSERT_COUNT` are validated before persistence so malformed data produces an actionable authoring error.
